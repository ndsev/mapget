...
{
    // If the field dict exists already, delete the entry.
    std::string previousFieldDict;
    rocksdb::WriteBatch batch;
    if (db_->Get(
               read_options_,
            ...
            .ok())
    {
        batch.Delete(column_family_handles_[COL_FIELD_DICTS], sourceNodeId);
    }
    batch.Put(column_family_handles_[COL_FIELD_DICTS], sourceNodeId, v);
    rocksdb::Status status = db_->Write(write_options_, &batch);
    if (!status.ok()) {
        throw logRuntimeError(stx::format("Error writing to database: {}", status.ToString()));
    }
