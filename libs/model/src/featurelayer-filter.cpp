...
public:
    ...
    {
        ++iterations_;
        return iterations_ % CancellationCheckBatch == 0 &&
            boundary();
    }
...
{
    explicit FeatureArrayViewStorage(
...
class FeatureArrayView final
    : public simfil::MandatoryDerivedModelNodeBase<
          FeatureArrayViewStorage>
{
public:
    explicit FeatureArrayView(
        std::vector<model_ptr<Feature>> features,
        simfil::detail::mp_key key)
        : simfil::MandatoryDerivedModelNodeBase<
              FeatureArrayViewStorage>(
              std::make_shared<FeatureArrayViewStorage>(
                  std::move(features)),
    ...
    explicit FeatureArrayView(
        ...
        simfil::detail::mp_key key)
        : simfil::MandatoryDerivedModelNodeBase<
              FeatureArrayViewStorage>(node, key)
    {}
    ...
    [[nodiscard]] simfil::ModelNode::Ptr at(
        int64_t index) const override
    {
        if (index < 0 ||
            static_cast<size_t>(index) >=
                model().features_.size())
        {
            return {};
        }
        return simfil::ModelNode::Ptr(
            model().features_[
                static_cast<size_t>(index)]);
    }
    ...
    bool iterate(
        IterCallback const& callback) const override
    {
...
FeatureArrayViewStorage::resolve(
    simfil::ModelNode const& node,
    ResolveFn const& callback) const
{
    auto resolved =
        simfil::model_ptr<FeatureArrayView>::make(node);
    callback(*resolved);
    return {};
}
...
public:
    using ProgramKey =
        std::tuple<std::string, bool, simfil::SchemaId>;
    ...
    {
        std::map<ProgramKey, std::vector<simfil::Value>>
            values_;
    };
    ...
    {
        ProgramKey key = std::make_tuple(
            std::string(expression),
            anyMode,
            rootSchema);
        if (valueCache && !collectDiagnostics) {
            if (auto cached = valueCache->values_.find(key);
                cached != valueCache->values_.end())
            {
                Result result;
        ...
        if (valueCache &&
            !collectDiagnostics &&
            result.traces_.empty())
        {
            valueCache->values_.emplace(
                std::move(key),
                result.values_);
        }
        return result;
    ...
    [[nodiscard]] std::shared_ptr<StringPool> const& strings() const
    {
        return strings_;
    }
...
{
    auto sourceStrings =
        std::dynamic_pointer_cast<StringPool>(sourceLayer.strings());
    if (!sourceStrings) {
    ...
    return EvaluatorContext{
        std::make_unique<FilterEvaluator>(
            std::move(strings),
...
simfil::ModelNode::Ptr contextWithBindings(
    ...
    std::span<BindingField const> bindings)
{
    ...
    auto context =
        simfil::model_ptr<simfil::OverlayNode>::make(simfil::Value::field(root));
...
LayerSchema::SearchQueryRequestedScope requestedScopeForNormalization(
    FeatureLayerFilterScope scope)
{
    ...
    case FeatureLayerFilterScope::Auto:
        return LayerSchema::SearchQueryRequestedScope::Auto;
    case FeatureLayerFilterScope::Relation:
        break;
    }
    return LayerSchema::SearchQueryRequestedScope::Feature;
}

FeatureLayerFilterScope concreteFilterScope(
    LayerSchema::SearchQueryConcreteScope scope)
{
...
tl::expected<void, simfil::Error> validateRequest(
    TileFeatureLayer const& sourceLayer,
    FeatureLayerFilterRequest const& request)
{
    ...
    auto const canValidateTypes =
        layerInfo && !layerInfo->featureTypes_.empty();
    for (auto const& channel : request.channels_) {
        ...
        if (!channel.group_ &&
            channel.scope_ == FeatureLayerFilterScope::Feature &&
        ...
        if (channel.group_) {
            if (channel.scope_ != FeatureLayerFilterScope::Feature ||
                !channel.featureFields_.empty() ||
                channel.entryFilter_)
            {
                return tl::unexpected(simfil::Error{
                    simfil::Error::InvalidArguments,
                    fmt::format(
                        "Point-grid channel '{}' requires feature scope, empty featureFields, and no entryFilter.",
                        channel.channelId_),
                });
            }
                ...
                size.z <= 0.0)
            {
                return tl::unexpected(simfil::Error{
                    simfil::Error::InvalidArguments,
                    fmt::format(
                        "Point-grid channel '{}' requires finite origin and positive finite cellSize components.",
                        channel.channelId_),
                });
            }
        }
        for (auto const& featureType : channel.featureTypes_) {
            ...
            if (canValidateTypes &&
                !layerInfo->getTypeInfo(featureType, false))
            {
                return tl::unexpected(simfil::Error{
                    simfil::Error::InvalidArguments,
                    fmt::format(
...
{
    if (values.empty() ||
        values.front().isa(simfil::ValueType::Undef) ||
    ...
    if (value.isa(simfil::ValueType::Bool) ||
        value.isa(simfil::ValueType::Int) ||
        value.isa(simfil::ValueType::Float))
    {
        return value;
    }
    if (value.isa(simfil::ValueType::String)) {
        return simfil::Value::make(
            value.as<simfil::ValueType::String>());
    }
    ...
    FilterEvaluator::EvaluationCache* valueCache = nullptr)
{
    ...
    for (auto const& expression : expressions) {
        ...
        mergeTraces(traces, std::move(result->traces_));
        values.push_back(scalarProjection(
    ...
    std::span<BindingField const> bindings)
{
    auto context = simfil::model_ptr<simfil::OverlayNode>::make(
        simfil::Value::field(*attribute));
    context->set(
        StringPool::OverlayNameStr,
        simfil::Value::make(std::string(attribute->name())));
    context->set(
        StringPool::OverlayFeatureStr,
        simfil::Value::field(*feature));
    context->set(
        StringPool::OverlayLayerStr,
        simfil::Value::make(std::string(layerName)));
    context->set(
        StringPool::OverlayAttributeIndexStr,
        simfil::Value::make(
            static_cast<int64_t>(attributeIndex)));
    context->set(
    ...
    context->set(
        StringPool::OverlayHasValidityStr,
        simfil::Value::make(hasValidity));
...
{
    auto const typeIndex =
        static_cast<std::underlying_type_t<GeomType>>(geometry->geomType());
    if (typeIndex >= 32 ||
...
std::optional<int64_t> pointGroupCoordinate(
    double coordinate,
    double origin,
    double cellSize)
{
    auto const value =
        std::floor((coordinate - origin) / cellSize);
    ...
    IssueAccumulator& issues)
{
    ...
    geometries->forEachGeometry(
        [&](model_ptr<Geometry> const& geometry) {
            auto const thisGeometryOrdinal =
                geometryOrdinal++;
            ...
            for (uint32_t pointOrdinal = 0;
                 pointOrdinal < geometry->numPoints();
                 ++pointOrdinal)
            {
                auto const point =
                    geometry->pointAt(pointOrdinal);
                auto x = pointGroupCoordinate(
                    point.x,
                    group.origin_.x,
                    group.cellSize_.x);
                auto y = pointGroupCoordinate(
                    point.y,
                    group.origin_.y,
                    group.cellSize_.y);
                auto z = pointGroupCoordinate(
                    point.z,
                    group.origin_.z,
                    group.cellSize_.z);
                if (!x || !y || !z) {
    ...
    for (auto& [_, member] : membersByCell) {
        channel.pointGroupMembers_.push_back(
            std::move(member));
    }
}
    ...
    IssueAccumulator& issues)
{
    ...
    if (auto const& pattern =
            channel.definition_.relation_->relationNamePattern_)
    {
        try {
            namePattern.emplace(
                *pattern,
    ...
    std::deque<RelationRoot> pending(
        channel.relationRoots_.begin(),
        channel.relationRoots_.end());
    ...
    while (!pending.empty()) {
        ...
        auto const sourceIdentity =
            current.feature_->id()->toString();
        if (!visitedFeatures.insert(sourceIdentity).second) {
            continue;
        }

        for (uint32_t relationOrdinal = 0;
             relationOrdinal <
                 current.feature_->numRelations();
             ++relationOrdinal)
        {
            auto relation =
                current.feature_->getRelation(
                    relationOrdinal);
            if (!relation ||
                (namePattern &&
                 !std::regex_match(
            ...
            auto localTarget = sourceLayer.find(
                targetId->typeId(),
    ...
    bool* downgradedGltfNodeIndex = nullptr)
{
    auto const targetType =
        source->geomType() == GeomType::GltfNodeIndex &&
            !preserveGltfNodeIndex
        ? GeomType::AABB
        : source->geomType();
    auto copied = target.newGeometry(
        targetType,
        std::max<size_t>(1, source->numPoints()),
        false);
    ...
    case GeomType::GltfNodeIndex:
        if (preserveGltfNodeIndex) {
            ...
            copied->setGltfNodeBounds(
                source->gltfNodeAabbOrigin(),
                source->gltfNodeAabbSize());
        }
        else {
            copied->setAabb(
                source->gltfNodeAabbOrigin(),
    ...
    default:
        ...
        if (source->geomType() == GeomType::Polygon &&
            source->numPolygonRings() > 1)
        {
            std::vector<uint32_t> ringStarts;
            ringStarts.reserve(source->numPolygonRings());
            for (uint32_t index = 0;
                 index < source->numPolygonRings();
                 ++index)
            {
    ...
    bool* downgradedGltfNodeIndex = nullptr)
{
    ...
    source->forEachGeometry([&](model_ptr<Geometry> const& geometry) {
        if (geometrySelected(geometry, geometryTypes, geometryName)) {
            copied->addGeometry(copyGeometry(
                target,
                geometry,
                preserveGltfNodeIndex,
    ...
    std::optional<std::string> const& geometryName)
{
    ...
    auto const typeIndex =
        static_cast<std::underlying_type_t<GeomType>>(source.geomType_);
    if (source.points_.empty() ||
        typeIndex >= 32 ||
        (geometryTypes & (uint32_t{1} << typeIndex)) == 0 ||
        (geometryName &&
         (!sourceName || *sourceName != *geometryName)))
    {
        return copied;
    }

    auto geometry = target.newGeometry(
        source.geomType_,
        std::max<size_t>(1, source.points_.size()),
        false);
    ...
    default:
        ...
        if (source.geomType_ == GeomType::Polygon &&
            source.polygonRingStarts_.size() > 1)
        {
            geometry->setPolygonRingStarts(
                source.polygonRingStarts_);
        }
        break;
    }
    ...
    IssueAccumulator& issues)
{
    ...
    try {
        ...
        issues.add(
            ...
            Scope::Attribute,
            fmt::format(
                "Could not compute validity geometry: {}",
                exception.what()));
...
{
    return target.newFeatureId(
        source->typeId(),
        source->keyValuePairs(),
        source->externalMapId());
}

std::vector<simfil::ModelNode::Ptr> materializeValues(
    TileSubsetLayer& target,
    std::vector<simfil::Value> const& values)
{
    ...
    IssueAccumulator& issues)
{
    ...
    for (auto const& candidate : channel.featureCandidates_) {
        auto values = materializeValues(
            target,
            candidate.featureValues_);
    ...
    for (auto const& candidate : channel.attributeCandidates_) {
        auto hostValues = materializeValues(
            target,
            candidate.hostValues_);
...
std::string stableFeatureIdentity(
    model_ptr<Feature> const& feature)
{
    return fmt::format(
        "{}:{}",
        MapTileKey(feature->model()).toString(),
        feature->id()->toString());
}

std::string directedRelationIdentity(
    ...
    std::span<BindingField const> bindings)
{
    ...
    context->set(
        ...
        simfil::Value::field(*descriptor.source_));
    context->set(
        StringPool::OverlayTargetStr,
        simfil::Value::field(*descriptor.target_));
    context->set(
        StringPool::OverlayTwowayStr,
        simfil::Value::make(twoway));
    context->set(
    ...
    model_ptr<GeometryCollection> const& fallback)
{
    ...
    auto result =
        target.newGeometryCollection(
            std::max<uint32_t>(1, validities->size()),
            false);
    for (uint32_t index = 0;
         index < validities->size();
         ++index)
    {
        ...
        try {
            std::string error;
            auto computed = validity->computeGeometry(
                feature->geomOrNull(),
                &error);
            if (!error.empty()) {
                issues.add(
...
{
    auto const [leftLongitude, leftLatitude] =
        left.tileId_.southWestWgs84();
    auto const [rightLongitude, rightLatitude] =
        right.tileId_.southWestWgs84();
    ...
    std::span<FeatureLayerFilterRoot const> exactRoots)
{
    ...
    auto const featureIdParts =
        castToKeyValue(featureId->keyValuePairs());
    std::optional<size_t> firstOrdinal;
    for (auto const& root : exactRoots) {
        auto const identityMatches =
            !root.canonicalFeatureId_.empty()
                ? root.canonicalFeatureId_ == featureId->toString()
                : root.typeId_ == featureId->typeId() &&
                    root.featureId_ == featureIdParts;
        if (root.tileId_ == featureKey.tileId_ &&
            identityMatches &&
            (!firstOrdinal ||
             root.requestOrdinal_ < *firstOrdinal))
        {
...
filterFeatureLayerSource(
    ...
    FeatureLayerFilterCancellationCheck const&
        cancellationCheck)
{
    if (auto valid = validateRequest(sourceLayer, request); !valid) {
    ...
    auto evaluatorContext =
        makeFilterEvaluator(sourceLayer, request.bindings_);
    if (!evaluatorContext) {
    ...
    if (materializeOutput) {
        resultLayer = std::make_shared<TileSubsetLayer>(
            ...
            request.filterId_,
            request.generation_);
        resultLayer->setGeometryAnchor(
            sourceLayer.geometryAnchor());
        resultLayer->adoptSourceInfo(sourceLayer);
        resultLayer->addDependency(
            MapTileKey(sourceLayer),
            static_cast<uint32_t>(sourceLayer.size()));
        if (auto const& attachmentName =
                sourceLayer.glbAttachmentName())
        {
            resultLayer->setGlbAttachmentName(
                *attachmentName);
        }
    }
    ...
    channels.reserve(request.channels_.size());
    for (auto const& requested : request.channels_) {
        auto effective = requested;
        bool normalizationFailed = false;
        if (requested.rewrite_ ||
            requested.scope_ == FeatureLayerFilterScope::Auto)
        {
            ...
            if (registry) {
                ...
                if (!normalized) {
                    ...
                    effective.entryFilter_ =
                        std::move(normalized->normalizedQuery_);
                    effective.scope_ =
                        concreteFilterScope(normalized->concreteScope_);
                }
            }
            else if (effective.scope_ ==
        ...
        channels.push_back(ChannelState{
            ...
            output,
            std::set<std::string>(
                requested.featureTypes_.begin(),
                requested.featureTypes_.end()),
            normalizationFailed,
        });
    }
    ...
    auto const collectEntryFilterDiagnostics =
        request.channels_.size() == 1;
    ...
    for (auto const& feature : sourceLayer) {
        ...
        auto const thisFeatureOrdinal =
            sourceFeatureOrdinal++;
        auto const featureSchema =
            static_cast<simfil::ModelNode const&>(*feature).schema();
        auto featureContext = contextWithBindings(
            *feature,
             ...
             ++channelIndex)
        {
            ...
            if (channel.filterCompilationFailed_ ||
                !featureTypeAllowed(feature, channel))
            {
                continue;
            }
            if (!materializeOutput &&
                !channel.definition_.group_)
            {
                continue;
            }
            if (channel.definition_.scope_ ==
            ...
            if (channel.definition_.scope_ ==
                FeatureLayerFilterScope::Relation)
            {
                channel.relationRoots_.push_back(
                    RelationRoot{
                        feature,
                        thisFeatureOrdinal,
                        false,
            ...
            attributeLayers->forEachLayer(
                [&](std::string_view layerName,
                    model_ptr<AttributeLayer> const& attributeLayer)
            {
                    ...
                    [&](model_ptr<Attribute> const& attribute)
                {
                            ...
                            uint32_t validityCount)
                    {
                        ...
                        auto context = makeAttributeContext(
                            ...
                            validityCount,
                            evaluatorContext->bindingFields_);
                        FilterEvaluator::EvaluationCache
                        ...
                        channel.attributeCandidates_.push_back(
                            AttributeCandidate{
                                feature,
                                attribute,
                                validity,
                    ...
                    if (validities && validities->size() > 0) {
                             ...
                             ++index)
                        {
                            ...
                            evaluateAttributeCandidate(
                                node
                                    ? attribute->model().resolve<Validity>(
                                          node)
                                    : model_ptr<Validity>{},
                                true,
                                index,
    ...
    if (materializeOutput && !exactRoots.empty()) {
        for (auto const& root : exactRoots) {
            ...
            for (auto& channel : channels) {
                ...
                auto context = contextWithBindings(
                    *feature,
                    evaluatorContext->bindingFields_);
         ...
         ++channelIndex)
    {
        ...
        pointGroupMembers.insert(
            ...
            std::make_move_iterator(
                channel.pointGroupMembers_.begin()),
            std::make_move_iterator(
                channel.pointGroupMembers_.end()));
        relationDescriptors.insert(
            relationDescriptors.end(),
            std::make_move_iterator(
                channel.relationDescriptors_.begin()),
            std::make_move_iterator(
                channel.relationDescriptors_.end()));
    }
...
{
    using GroupId =
        std::pair<size_t, FeatureLayerPointGroupKey>;
    std::map<
        GroupId,
        std::vector<FeatureLayerPointGroupMember>>
        groups;
    ...
    for (auto const& member : members) {
        ...
        if (member.channelIndex_ >=
                request.channels_.size() ||
        ...
        groups[
            {member.channelIndex_, member.key_}]
            .push_back(member);
    }
    ...
    for (auto& [groupId, groupMembers] : groups) {
        ...
        auto const& definition =
            request.channels_[channelIndex];
        ...
        std::ranges::sort(
            ...
            [&](auto const& left, auto const& right) {
                ...
                return std::tie(
                           leftIdentity,
                           left.geometryOrdinal_,
                           left.pointOrdinal_) <
                       std::tie(
                           rightIdentity,
                           right.geometryOrdinal_,
                           right.pointOrdinal_);
            });
        ...
        auto outputChannel =
            outputLayer.at(channelIndex);
        if (!outputChannel ||
            outputChannel->scope() != Scope::Group ||
        ...
        auto values = evaluateFields(
            ...
            *context,
            ...
            issues);
        auto groupKey =
            outputLayer.newArray(3, true);
        groupKey->append(key.x_);
        groupKey->append(key.y_);
        groupKey->append(key.z_);

        auto geometry =
            outputLayer.newGeometryCollection(
                1,
                true);
        auto pointGeometry =
            outputLayer.newGeometry(
                GeomType::Points,
                1,
                true);
        pointGeometry->append(
            representative.representativePoint_);
        ...
        cancellationCheck)
{
    ...
    for (size_t channelIndex = 0;
         channelIndex < request.channels_.size();
         ++channelIndex)
    {
        if (cancellation.boundary()) {
        ...
        auto outputChannel =
            outputLayer.at(channelIndex);
        if (!outputChannel ||
            outputChannel->scope() != Scope::Relation ||
            outputChannel->channelId() !=
                definition.channelId_)
        {
            return tl::unexpected(simfil::Error{
        ...
        std::vector<
            FeatureLayerRelationDescriptor const*>
            channelDescriptors;
        for (auto const& descriptor : descriptors) {
            ...
            if (descriptor.channelIndex_ ==
                    channelIndex &&
                descriptor.source_ &&
                descriptor.relation_ &&
                descriptor.target_)
            {
                channelDescriptors.push_back(
                    &descriptor);
            }
        }
        std::ranges::sort(
            channelDescriptors,
            [](auto const* left, auto const* right) {
                return directedRelationIdentity(*left) <
                    directedRelationIdentity(*right);
            });
             ...
             channelDescriptors)
        {
            ...
            auto const directedId =
                directedRelationIdentity(*descriptor);
            ...
            if (definition.relation_->mergeTwoway_) {
                     ...
                     channelDescriptors)
                {
                    ...
                    auto const candidateId =
                        directedRelationIdentity(
                            *candidate);
                    if (consumedRelations.contains(
                            candidateId) ||
                        stableFeatureIdentity(
                            candidate->source_) !=
                            targetIdentity ||
                ...
                if (!twoway) {
                         ...
                         ++ordinal)
                    {
                        ...
                        auto reverseTarget =
                            reverse->target();
                        if (!reverseTarget ||
                            reverseTarget->toString() !=
                                descriptor->source_
                                    ->id()->toString())
                        {
                            continue;
                        }
                        auto candidateId = fmt::format(
                            "{}#{}",
                            targetIdentity,
                            ordinal);
                        if (!consumedRelations.insert(
                                candidateId).second)
                        {
                            continue;
                        }
            ...
            if (twoway && exactRoots.empty()) {
                ...
                auto const targetKey =
                    descriptor->targetTileKey_
                        .value_or(MapTileKey(
                            descriptor->target_
                                ->model()));
                auto const sourceFeatureIdentity =
                    descriptor->source_->id()
                        ->toString();
                auto const targetFeatureIdentity =
                ...
                auto ownerRootOrdinal =
                    descriptor->rootOrdinal_;
                if (auto sourceRoot = exactRootOrdinal(
                        descriptor->source_,
                        outputLayer,
                        exactRoots))
                {
                ...
                if (auto targetRoot = exactRootOrdinal(
                        descriptor->target_,
                        outputLayer,
                        exactRoots))
                {
        ...
        {
            auto const identity =
                stableFeatureIdentity(feature);
            if (auto found =
                    endpointEntries.find(identity);
                found != endpointEntries.end())
            {
                return found->second;
            }
            ...
            if (downgradedGltfNodeIndex) {
                issues.add(
                    ...
                    "<relation-geometry>",
                    Scope::Relation,
                    "Foreign GLTF node geometry was downgraded to its AABB because tile attachments are tile-local.");
            }
            endpointEntries.emplace(
                identity,
                entry);
            return entry;
        };
        ...
        for (auto const& emission : emissions) {
            ...
            auto context = makeRelationContext(
                descriptor,
                emission.twoway_,
                evaluatorContext->bindingFields_);
            auto const relationSchema =
                static_cast<simfil::ModelNode const&>(
                    *descriptor.relation_).schema();
            auto entryMatch = evaluateFilter(
                *evaluatorContext->evaluator_,
            ...
            auto sourceEntry =
                endpointEntry(descriptor.source_);
            if (!sourceEntry) {
                return tl::unexpected(
                    sourceEntry.error());
            }
            auto targetEntry =
                endpointEntry(descriptor.target_);
            if (!targetEntry) {
                return tl::unexpected(
                    targetEntry.error());
            }
    ...
    FeatureLayerFilterRequest const& request)
{
            ...
            }))
    {
        return tl::unexpected(simfil::Error{
            ...
            "filterFeatureLayer() is source-local; point groups and stored relations require the service executor.",
        });
    }

    auto sourceResult = filterFeatureLayerSource(
        sourceLayer,
        request,
        true);
    if (!sourceResult) {
    ...
    sourceResult->layer_->setTraces(
        std::move(sourceResult->traces_));
    sourceResult->layer_->setDiagnostics(
