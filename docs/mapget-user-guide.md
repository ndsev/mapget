...
- [**Configuration Guide**](mapget-config.md) documents the YAML configuration file used with `--config`, the supported datasource types (`DataSourceHost`, `DataSourceProcess`, `GridDataSource`, `GeoJsonFolder`), generic datasource fields such as `enabled`, and optional UI-facing sections such as `http-settings`.
